...
export default function BattlePage({ battle, related, siteUrl }) {
  ...
  return (
    <div className="mx-auto max-w-3xl pb-10 pt-6 lg:max-w-4xl">
      ...
      The color itself now comes from whichever side is CURRENTLY
      LEADING's own real identity (getProductTint — the actual extracted
      logo color when one exists, same "Pecan AI's page is green because
      Pecan's logo is green" effect as the product page), not the earlier
      arbitrary getCoolTint(battle.id) hash. If the lead changes as more
      votes come in, the page's color follows it on the next load — this
      isn't recomputed client-side mid-session, only on each page load. */}
      <BattleCard
        battle={battle}
        theme={
          battle.status === "live" &&
          (!battle.ends_at || new Date(battle.ends_at) > new Date())
            ? getProductTint(
                battle.votes_a >= battle.votes_b ? battle.product_a : battle.product_b
              )
            : null
        }
      />

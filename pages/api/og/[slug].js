...
// Same lighten-toward-white math as lib/colorMath.js, duplicated here
// for the same reason as getAvatarTint above — this file intentionally
// avoids importing from lib/ (edge runtime, keeps the bundle minimal
// and avoids any risk of a non-edge-safe transitive import).
function lightenForBackground(hex, amount = 0.88) {
...
// Prefers a product's REAL extracted brand color (see lib/brandColor.js
// — populated server-side elsewhere, this file only reads it) over the
// name-hash tint, same resolver logic as lib/categoryIcons.js's
// getProductTint, kept in sync manually rather than imported.
function getProductTint(product) {
  if (product?.brand_color) {
    return {
      bg: lightenForBackground(product.brand_color),
      text: product.brand_color,
    };
  }
  return getAvatarTint(product?.name);
}
...
export default async function handler(req) {
  try {
    ...
    const { data: battle, error } = await supabase
      .from("battles")
      .select(
        "slug, status, votes_a, votes_b, votes_a_boost, votes_b_boost, views, clicks, question, created_by, product_a:product_a_id(name, logo_url, brand_color, category:category_id(name)), product_b:product_b_id(name, logo_url, brand_color)"
      )
      .eq("slug", slug)
      .single();

...
function BackfillBrandColors() {
  ...
  async function runBatch() {
    ...
    try {
      const res = await authedFetch("/api/admin", {
        method: "POST",
        body: JSON.stringify({ type: "backfill-brand-colors" }),
      });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.error || "Backfill batch failed");
      setLastResult(body);
      // Existing products won't get a real brand color just because
      // this code shipped — someone has to actually run extraction on
      // each one. Batched to avoid a serverless timeout on a large
      // catalog, so this keeps calling itself until nothing's left.
      if (body.remaining > 0) {
        runBatch();
      } else {
        setStatus("done");
      ...
      logError("emmybund.BackfillBrandColors.runBatch", err);
  ...
  return (
    <div className="mt-6 rounded-lg border border-line bg-white p-3">
      <h3 className="font-display text-xs uppercase tracking-wide">
        Brand color backfill
      </h3>
      <p className="mt-1 font-mono text-[10px] text-grayText">
        Products that existed before real logo-color extraction don't have
        one yet — run this once to fill them in (safe to re-run any time).
      </p>
      <button
        onClick={runBatch}
        disabled={status === "running"}
        className="mt-2 rounded-lg bg-ink px-3 py-1.5 font-mono text-[10px] font-bold text-white disabled:opacity-60"
      >
        {status === "running" ? "Running…" : "Run backfill"}
      </button>
      {lastResult && (
        <p className="mt-2 font-mono text-[10px] text-grayText">
          Last batch: {lastResult.succeeded} extracted, {lastResult.failed} failed,{" "}
          {lastResult.remaining} remaining.
        </p>
      )}
      {status === "done" && (
        <p className="mt-1 font-mono text-[10px] font-bold text-cornerB">All caught up.</p>
      )}
      {error && <p className="mt-1 font-mono text-[10px] text-cornerA">{error}</p>}
    </div>
  );
}

...
import { closeExpiredBattles } from "../lib/closeExpiredBattles";
...
// — see lib/closeExpiredBattles.js. Now shared (was previously
// homepage-only, which caused /battles and the homepage to disagree
// about which battles were actually still live — a battle could sit
// expired but still marked "live" until someone happened to load the
...
export async function getServerSideProps({ query }) {
  ...
  try {
    ...
    const { data: battles, error: battlesError } = await supabase
      .from("battles")
      .select(
        "id, slug, votes_a, votes_b, votes_a_boost, votes_b_boost, status, question, starts_at, ends_at, views, winner_id, clicks, created_by, product_a:product_a_id(id, name, slug, logo_url, brand_color, brand_text_color, category:category_id(name, slug)), product_b:product_b_id(id, name, slug, logo_url, brand_color, brand_text_color, category:category_id(name, slug))"
      )
      .eq("status", "live")
    ...
    let productsQuery = supabase
      .from("products")
      .select("id, name, slug, rating, wins, losses, category_id, logo_url, brand_color, brand_text_color, clicks, category:category_id(name, icon, slug)")
      .eq("status", "active")
    ...
    <div className="mx-auto max-w-6xl pb-12">
      ...
      {happeningNow.length > 0 && (
        <section className="px-5 py-8 md:px-8">
          ...
          <div className="flex flex-col divide-y divide-line rounded-xl border border-line bg-white shadow-[3px_3px_0_#2B2620]">
            {happeningNow.map((b) => {
                ...
                >
                  <div className="flex shrink-0 flex-col items-center gap-1">
                    <div className="flex items-center">
                      ...
                      <div
                        className="-ml-1.5 flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-line font-display text-[10px]"
...
function ProductSlot({
  ...
  const [groundedInSearch, setGroundedInSearch] = useState(false);
  ...
  async function fetchCompetitors() {
    ...
    try {
      ...
      setGroundedInSearch(Boolean(body.groundedInSearch));
    } catch (err) {
          ...
          {competitors === null ? (
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                {competitors.length > 0 && !groundedInSearch && (
                  <div className="font-mono text-[9px] text-grayText" title="Real-time search wasn't used for these — could be less current than usual.">
              ...
              <button
                ...
                className="mt-2 font-mono text-[11px] font-bold text-cornerB"
              >
                Don't see who you're looking for? Add them by URL
              </button>
            </div>
          )}
        </div>
...
function AddProductForm({ initialName, initialWebsiteUrl, categories, onCancel, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!websiteUrl.trim()) {
      setStatus("error");
      setMessage("Website URL is required — everything else can auto-fill from it.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <input
        ...
        placeholder="Website URL (required — everything else can auto-fill from this)"
      ...
      <input
        ...
        placeholder="Product name (optional — auto-detected from the site)"
        className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-sm text-ink outline-none placeholder:text-grayText"
      />

      <div>
        <div className="mb-1.5 font-mono text-[10px] uppercase text-grayText">
          Category (optional — auto-classified if left blank)
          {selectedCategory

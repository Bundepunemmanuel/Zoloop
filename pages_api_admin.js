      ...
      try {
        ...
        if (imgRes.ok) {
          ...
          if (extracted) {
            ...
            logInfo("api/admin.updateProductDetails.brandColor", "Extraction succeeded", {
              productId,
  ...
  try {
    ...
    for (const p of candidates ?? []) {
      try {
        ...
        clearTimeout(timeout);
        if (!imgRes.ok) {
          logWarn("api/admin.backfillBrandColors.item", "Logo image fetch failed", {
            productId: p.id,
            logoUrl: p.logo_url,
            status: imgRes.status,
          });
        ...
        const extracted = await extractBrandColor(buffer);
        if (!extracted) {
          // extractBrandColor already logs the underlying reason
          // (lib/brandColor.js) — this just ties it back to which
          // product it was for, since the library-level log doesn't
          // know that.
          logWarn("api/admin.backfillBrandColors.item", "Extraction returned null", {
            productId: p.id,
            logoUrl: p.logo_url,
          });
          failed++;
        ...
        if (updateError) {
          ...
          succeeded++;
          logInfo("api/admin.backfillBrandColors.item", "Backfilled one product", {
            productId: p.id,
            brandColor: extracted.brandColor,
          });
        }

...
export async function getServerSideProps({ params }) {
  try {
    ...
    const { data: battles, error: battlesError } = await supabase
      .from("battles")
      .select(
        "id, slug, status, votes_a, votes_b, votes_a_boost, votes_b_boost, views, winner_id, starts_at, ends_at, product_a_id, product_b_id, product_a:product_a_id(name, slug, logo_url, brand_color, brand_text_color), product_b:product_b_id(name, slug, logo_url, brand_color, brand_text_color)"
      )
    ...
    <div className="mx-auto max-w-2xl px-5 pb-10 pt-6 md:px-8 lg:max-w-3xl">
      ...
      <div className="flex flex-col gap-2 md:grid md:grid-cols-2 md:gap-3">
        {battles.map((b) => {
          ...
          const opponentTint = getProductTint(opponent);

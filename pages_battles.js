...
export async function getServerSideProps({ query }) {
  ...
  try {
    // BUG FIX: this page used to trust the raw `status` column with no
    // check on whether a battle's time had actually run out, which is
    // exactly why it was showing battles as "LIVE" that the homepage
    // had already correctly excluded — the two pages disagreed about
    // what "live" meant. Running the same sweep here keeps them
    // consistent.
